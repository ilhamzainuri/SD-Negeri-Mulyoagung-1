...
import { Plus, Edit2, Trash2, Image, Calendar, CheckCircle2, Clock, XCircle, Search, Filter, RotateCcw, X } from 'lucide-react';
...
export default function GaleriCrud({ currentUser }: GaleriCrudProps) {
  ...
  const availableCategories = Array.from(new Set(items.map((item) => item.kategori).filter(Boolean)));
  const availableStatuses = Array.from(new Set(items.map((item) => item.status_verifikasi).filter(Boolean)));
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesStatus = selectedStatus === 'ALL' || item.status_verifikasi === selectedStatus;
