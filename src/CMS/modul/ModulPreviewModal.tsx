...
export const ModulPreviewModal: React.FC<ModulPreviewModalProps> = ({ module, onClose }) => {
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl w-full max-w-5xl h-[92vh] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden text-left my-auto animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="bg-gradient-to-r from-teal-800 via-teal-900 to-slate-950 p-3.5 sm:p-5 text-white flex justify-between items-center shrink-0 border-b border-teal-700/50">
          <div className="flex items-center gap-3 overflow-hidden min-w-0">
            <div className="p-2.5 rounded-xl bg-teal-500/20 border border-teal-400/30 shrink-0 text-teal-300">
              <BookOpen size={20} />
            </div>
            <div className="overflow-hidden min-w-0">
              <h3 className="font-bold text-sm sm:text-base lg:text-lg leading-tight truncate text-white">
                {module.judul}
              </h3>
              <div className="flex flex-wrap items-center gap-2 text-[11px] sm:text-xs text-teal-200/90 mt-1">
                <span className="font-semibold text-teal-300">{module.mata_pelajaran}</span>
                <span>•</span>
                <span>{module.kelas}</span>
                <span>•</span>
                <span>Semester {module.semester}</span>
                <span>•</span>
                <span>TA {module.tahun_ajaran}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0 ml-3">
            {directUrl && (
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-teal-600/80 hover:bg-teal-600 text-white text-xs font-semibold backdrop-blur-md transition-all border border-teal-400/40 shadow-sm cursor-pointer"
        ...
        <div className="p-3 sm:p-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500 shrink-0">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="bg-teal-50 text-teal-700 border border-teal-200 dark:bg-teal-950/60 dark:text-teal-300 dark:border-teal-800 font-bold px-2.5 py-0.5 rounded-full">
              {module.kategori}
            </span>
            <span className="text-slate-600 dark:text-slate-400">
              Pengunggah: <strong className="text-slate-800 dark:text-slate-200">{module.uploader || 'Guru Pengampu'}</strong>
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
              className="px-4 py-1.5 rounded-xl bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold transition-colors cursor-pointer"
            >
              Tutup
            </button>
          </div>
